local gameManagement = {}
	...
	function gameManagement.startGame(player1, player2, mapName)
		...
		local intValues = game.ReplicatedStorage.intValues
					...
					for i,v in pairs (destructionNodes:GetChildren()) do
						...
						if v.Value == 2 then
							...
							if nodeDestructName == baseInfo.ASideBase.Value then
								...
								Events.warning:FireAllClients("B Side wins by destruction.")
							elseif nodeDestructName == baseInfo.BSideBase.Value then
								Events.endGame:FireAllClients()
								Events.warning:FireAllClients("A Side wins by destruction.")
							end
						end
					end
	...
	Events.warning.OnServerEvent:Connect(function(player, warningText)
		...
		if warningText:sub(1,11) ~= "A Side wins" or warningText:sub(1,11) ~= "B Side wins" then
			Events.warning:FireClient(player, warningText)  --Normal warning
		else 
			Events.warning:FireAllClients(warningText)      --Game end warning
		end
	...
	--Handles forfeit on the client side.
	Events.endGame.OnServerEvent:Connect(function()
		
		Events.endGame:FireAllClients()
		
	end)
	
	--Handles game decisions by abandonment
	game.Players.PlayerRemoving:Connect(function(player)
		
		--Checks if the game is occurring.
		if #game.Workspace.currentMap.octaNodes:GetChildren() > 0 then
			
			--Game decision by abandonment is valid.
			if (player.Team == Teams.A) then
				
				Events.warning:FireAllClients("B Side wins by abandonment.")
				Events.endGame:FireAllClients()
				
			elseif (player.Team == Teams.B) then
				
				Events.warning:FireAllClients("A Side wins by abandonment.")
				Events.endGame:FireAllClients()
